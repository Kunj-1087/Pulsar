...
export default function Home() {
  ...
  return (
    <main className="w-screen h-screen flex flex-col items-center justify-center bg-bg-base p-4 overflow-y-auto">
      <IdentityGate>
        {/* Offline Mode Info Block */}
        {isOfflineMode && (
          <div className="w-full max-w-[440px] mb-6 px-6 py-4 border border-border-default bg-bg-surface rounded-md">
            <div className="flex items-start gap-3">
              <div className="flex-1">
                <h2 className="type-uppercase-label text-fg-primary mb-2">
                  Offline LAN Mode
                </h2>
                <p className="font-sans text-small text-fg-muted leading-relaxed mb-3">
                  Running without internet. Share the URL or QR with devices on the same network.
                </p>
                <div className="bg-bg-elevated border border-border rounded px-3 py-2 mb-3">
        ...
        <Suspense fallback={
          <div className="w-full max-w-[440px] px-6 py-24 border border-border-default bg-bg-surface rounded-md flex flex-col items-center justify-center gap-4">
            <Spinner className="w-6 h-6 text-fg-primary" />
            <span className="type-terminal-msg text-fg-muted">loading<span className="animate-cursor-blink ml-0.5">_</span></span>
          </div>
        }>
          <RoomCreator />
        </Suspense>
